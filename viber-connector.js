      ...
      function(msg, callback) {
        try {
          if (msg.address) {
            ...
            logger.error(
              "ViberEnabledConnector: send - message is missing address."
            );
  ...
  ViberEnabledConnector.prototype.convertToViberMessages = function(message) {
    ...
    if (message.sourceEvent && message.sourceEvent.type) {
      switch (message.sourceEvent.type) {
        case "sticker":
          return [
            new VMessage.Sticker(
              message.sourceEvent.sticker_id,
              null,
              null,
              new Date(),
              ""
    ...
    if (
      ...
      if (message.text.button) {
        viberKb = {
          ...
          Buttons: [
            {
              ActionType: "reply",
              ActionBody: message.text.button,
      ...
      return [new VMessage.Text(message.text.message, viberKb)];
    }
        ...
        case "application/vnd.microsoft.card.hero":
          if (
            ...
            message.attachments.forEach(element => {
              ...
              if (attachment.buttons[0].type && attachment.images[0].url) {
                ...
                allAttachments.push({
                  ...
                  Text:
                    (attachment.title
                      ? '<font size="4" color="#323232"><b>' +
                        attachment.title +
                        "</b></font>"
                      : "") +
                    (attachment.subtitle
                      ? '<font size="2" color="#4C4C4C"><br>' +
                        attachment.subtitle +
                        "</font>"
                      : "") +
                    (attachment.text
                  ...
                  allAttachments.push({
                    ...
                    Text:
                      '<b><font size="4" color="#FFFFFF">' +
                        attachment.buttons[i].title || "" + "</font></b>",
                    BgColor: "#7536D1",
                    TextSize:
                      attachment.buttons.length === 1 ? "large" : "small",
                    TextVAlign: "middle",
                    TextHAlign: "middle"
                  });
                }
            ...
            if (message.text && message.text != "") {
              return [
                new VMessage.Text(message.text),
                new VMessage.RichMedia(RICH_MEDIA)
              ];
            } else {
  ...
  ViberEnabledConnector.prototype.postMessage = function(message, cb) {
    ...
    if (message.type === "typing") {
      ...
      viberMessages
        .reduce(
          (promise, vMessage) =>
            promise.then(() => this.viberBot.sendMessage(profile, vMessage)),
          Promise.resolve()
        )
        .then(() => cb())
